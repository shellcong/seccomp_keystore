...
public:
	...
	int seccomp_client(const char* filename, int flag){
		...
		// start to reveive the file descriptor
    ...
    ResponseCode readBlob(const char* filename, AES_KEY *aes_key, State state) {
		...
		int in = seccomp_client(filename, O_RDONLY);	
